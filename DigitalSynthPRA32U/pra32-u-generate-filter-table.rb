...
def generate_filter_lpf_table(res_idx, name, q)
  $file.printf("uint16_t g_filter_lpf_table_%s[] = {\n  ", name)
  (0..DATA_BYTE_MAX).each do |i|
    ...
    $file.printf("(uint16_t) %5d, (uint16_t) %+6d, (uint16_t) %+6d,", b_2_over_a_0_gain, a_1_over_a_0, a_2_over_a_0)
    if i == DATA_BYTE_MAX
      $file.printf("\n")
    elsif i % 2 == (2 - 1)
      $file.printf("\n  ")
...
$file.printf("uint16_t* g_filter_lpf_tables[] = {\n  ")
(0..8).each do |res_idx|
