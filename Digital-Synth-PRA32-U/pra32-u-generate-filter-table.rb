...
def generate_filter_lpf_table(res_idx, name, q)
  ...
  (0..DATA_BYTE_MAX * 2).each do |i|
    f_idx = [[24 * 2, i - 1 * 2].max, 120 * 2].min
    ...
    b_2_over_a_0_gain = (input_gain * (b_2 / a_0) * (1 << ((FILTER_TABLE_FRACTION_BITS + 2) * 2))).floor.to_i
    ...
    $file.printf("(uint32_t) %+11d, ((uint32_t) %+6d << 16) + %+6d,", b_2_over_a_0_gain, a_1_over_a_0, a_2_over_a_0)
    if i == DATA_BYTE_MAX * 2
      $file.printf("\n")
    elsif i % 2 == (2 - 1)
      $file.printf("\n  ")
    else
...
$file.printf("uint32_t* g_filter_lpf_tables[] = {\n  ")
(0..8).each do |res_idx|
